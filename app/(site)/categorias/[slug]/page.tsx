  ...
  return (
    <section className="mx-auto grid max-w-6xl gap-10 px-4 py-12 sm:px-6 lg:px-8">
      <div className="max-w-3xl">
        ...
        {category.description ? <p className="mt-4 text-lg leading-8 text-coffee/70 dark:text-paper/70">{category.description}</p> : null}
      </div>
      ...
      {posts.length > 0 ? (
        ...
        <div className="rounded-card border border-beige/70 p-8 dark:border-paper/10">
          ...
          <p className="mt-3 text-coffee/70 dark:text-paper/70">Esta categoria esta pronta para receber novos textos.</p>
        </div>
      )}
    </section>
  )
